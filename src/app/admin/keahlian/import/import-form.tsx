...
import { importKeahlianAction, type ImportRowError } from "./actions";

export function KeahlianImportForm() {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    start(async () => {
      ...
      fd.append("file", file);
      const res = await importKeahlianAction(fd);
      if (res.ok) {
        toast.success(`Berhasil import ${res.imported} keahlian.`);
        setFile(null);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <Button asChild variant="outline">
          <a href="/admin/keahlian/import/template" download>
            <Download className="h-4 w-4" />
            Unduh template
          </a>
        </Button>
        <p className="text-sm text-muted-foreground">
          Unduh template, isi sheet <strong>Keahlian</strong>, lalu upload di sini.
        </p>
      </div>
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            Maks 5 MB · 1000 baris · nama dicek unik (case-insensitive) terhadap
            data yang sudah ada.
          </p>
        </div>
