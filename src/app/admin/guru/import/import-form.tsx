...
import { importGuruAction, type ImportRowError } from "./actions";

export function GuruImportForm() {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    start(async () => {
      ...
      fd.append("file", file);
      const res = await importGuruAction(fd);
      if (res.ok) {
        toast.success(`Berhasil import ${res.imported} guru.`);
        setFile(null);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <Button asChild variant="outline">
          <a href="/admin/guru/import/template" download>
            <Download className="h-4 w-4" />
            Unduh template
          </a>
        </Button>
        <p className="text-sm text-muted-foreground">
          Unduh template, isi sheet <strong>Guru</strong>, lalu upload di sini.
        </p>
      </div>
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            Maks 5 MB · 500 baris · password awal semua guru{" "}
